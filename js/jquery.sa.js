...
String.prototype.htmlDecode = function() {
    return this.replace(/&lt;/mg, "<").replace(/&gt;/mg, ">").replace(
            /&quot;/mg, "\"").replace(/&amp;/mg, "&")
}
        ...
        if (selector instanceof $sa.Snippet) {
            ...
            switch (selector.charAt(0)) {
                ...
                case "[": // [field[value]]
                    ...
                    if (endOfFieldPos != -1 && endOfValuePos != -1
                            && endOfValuePos > endOfFieldPos) {
                        this.snippets = $sa.store.getSnippetsByField(selector
                                .substring(1, endOfFieldPos), selector.substring(
                                endOfFieldPos + 1, endOfValuePos))
                    }
                    break;
                default: // snippetname
            ...
            switch (selector.charAt(0)) {
                ...
                case "#": // #tagname
                    for (t = 0; t < this.snippets.length; t++) {
                        if (this.snippets[t].tags
                                && $.inArray(selector.substr(1),
                                this.snippets[t].tags) !== -1) {
                ...
                case "[": // [field[value]]
                    ...
                    if (endOfFieldPos != -1 && endOfValuePos != -1
                            && endOfValuePos > endOfFieldPos) {
                        for (t = 0; t < this.snippets.length; t++) {
                            if (this.snippets[t][selector.substring(1,
                                    endOfFieldPos)] === selector.substring(
                                    endOfFieldPos + 1, endOfValuePos)) {
                                filtered.push(this.snippets[t])
                            }
                        }
...
$sa.fn.navigate = function(options) {
    ...
    options = $.extend({}, defaults, options)
    options.template = options.template ? options.template : $sa.store
            .getSnippet(options.templateTitle)
    this.each(function() {
...
$sa.utils.parseFields = function(text) {
    ...
    for (var t = 0; t < lines.length; t++) {
        ...
        if (c != -1)
            $sa.utils.addField(fields, $.trim(lines[t].substring(0, c)), $
                    .trim(lines[t].substring(c + 1)))
    }
    return fields
};
...
$sa.utils.renderObject = function(object, excludeNames) {
    ...
    for (var n in object) {
        if (!excludeNames || $.inArray(n, excludeNames) == -1) {
            ...
            $("<span/>").addClass("snippet_dataValue").text(
                    object[n].toString()).appendTo(wrapper)
            j = j.add(wrapper)
        }
    }
    return j
...
$sa.utils.renderArray = function(array, excludeNames) {
    ...
    for (var t = 0; t < array.length; t++) {
        if (!excludeNames || $.inArray(array[t].name, excludeNames) == -1) {
            var wrapper = $("<div/>").addClass("snippet_dataRow")
            $("<span/>").addClass("snippet_dataName").text(array[t].name)
                    .appendTo(wrapper)
            $("<span/>").addClass("snippet_dataValue").text(array[t].value)
                    .appendTo(wrapper)
            j = j.add(wrapper)
        }
...
$sa.store = {}
var snippets = {};
var notificationCallbacks = [] // Array of {callback: function(), selector:
// selector}
var triggeredNotifications = [] // Subarray of notifications that have already
// been tripped
var untriggeredNotifications = [] // Subarray of notifications that have not
// yet been tripped
...
$sa.store.registerModification = function(snippet) {
    ...
    if (!notificationTimer) {
        ...
        untriggeredNotifications = notificationCallbacks.slice() // Make a
        // shallow
        // copy of
        // the
        // notifications
...
$sa.store.saveStore = function(store) {
    ...
    var html = $("<html/>").append(
            $("<head/>").append($("<title/>").text(title)).append(
            $("<style/>", {
        type: "text/css"
    }).text(".snippet { display: none; }"))).append(
            $("<body/>"))
}
...
$sa.serialisers.html.saveSnippet = function(snippet) {
    ...
    return $("<div/>", {
        ...
        "class": "snippet_title ui-widget-header"
    })).append($("<abbr/>", {
        "class": "snippet_created published",
        title: snippet.created
    }).text(snippet.created)).append($("<abbr/>", {
        "class": "snippet_modified updated",
        title: snippet.updated
    }).text(snippet.created)).append(tags).append($("<div/>", {
        "class": "snippet_body ui-widget-content"
    }).text(snippet.text))
}
...
$sa.display.chooseDisplayPoint = function(snippet, options) {
    ...
    if (elemSnippet === null) {
        ...
        containers.each(function(i) {
            if (elemSnippet === null) {
                elemSnippet = $sa.display.createSnippetFrameInContainer(
                        $(this), snippet, options)
            }
        })
    }
    if (elemSnippet === null) {
...
$sa.display.createSnippetFrameInContainer = function(elemContainer, snippet,
        options) {
    var elemSnippet = null
    elemSnippet = $sa.display.insertSnippetFrameInContainer(elemContainer,
            snippet, options)
    return elemSnippet
}
...
$sa.display.insertSnippetFrameInContainer = function(elemContainer, snippet,
        options) {
    var elemSnippet = null
    elemSnippet = $("<div/>").appendTo(
            elemContainer.find(".snippetContent").eq(0))
    return elemSnippet
}

$('.transclude').livequery(
        function() {
            $(this).addClass('macro')
            if (document.running) {
                var title = $(this).attr("data-snippet")
                var templateTitle = $(this).attr("data-template")
                var template = templateTitle ? $sa.store
                        .getSnippet(templateTitle) : null
            ...
            $(this).click(
                    function(e) {
                        ...
                        if (!target) {
                            ...
                            target = opts ? encodeSubject(opts.param)
                                    : undefined
                        }
                        if (target) {
                            ...
                            res.each(function() {
                                var triple = "<" + this.s.value.toString()
                            ...
                            // document.kb = document.kb.except(triples) // we
                            // should all ?s ?p <target> too?
...
$('.link').livequery(function() {
    ...
    $(this).click(function(e) {
        ...
        if (persist) {
            ...
            if (current.length > 0)
                current.hide("puff", {}, "normal", function() {
                    $(this).remove()
        ...
        function() {
            $(this).click(
                    function(e) {
                        ...
                        var opt = snippetframe.data('options')
                        var subject = opt ? encodeSubject(opt.param)
                                : undefined
                        var target = $(this).attr("data-link-target")
                        ...
                        var templateText = $sa.store.getSnippetById(template)
                                .text()
                        var resource = "#" + Math.uuid(17)
                        var substituted = templateText.replace(/#subject/,
                                resource)
                        ...
                        if (property)
                            document.kb.add(subject + " " + property + " <"
                                    + resource + ">")
                        if (inverseProperty)
                            document.kb.add("<" + resource + "> "
        ...
        function() {
            $(this)
                    ...
                    function(e) {
                        ...
                        var kb = document.kb
                        if (document.voc)
                            kb = kb.add(document.voc)
                        var props = kb
                                .where("?p rdfs:subPropertyOf gr:quantitativeProductOrServiceProperty")
                        props = props
                        ...
                        dia
                            ...
                            buttons: {
                                ...
                                "Add": function() {
                                    ...
                                    document.kb.setDirty = true
                                    $sa.store
                                            .registerModification('Properties')
                                    $(this).dialog(
                                            "close")
                                }
                            },
...
$('.repositoryCollections').livequery(
        function() {
            var self = this
            var snippetframe = $(this).parents('.dataTag:first')
            var opt = snippetframe.data('options')
            ...
            $.ajax({
                ...
                success: function(data) {
                    $(self).find('[data-view]').each(
                            function() {
                                var self = this
                                data.forEach(function(d) {
                                    var e = $(self).clone()
                                    e.find('td').html(d).attr('data-parameter',
                                            d).appendTo($(self).parent())
                                })
                                $(self).hide()
                            })
                },
...
$('.repositoryResources').livequery(
        function() {
            ...
            snippetframe.bind("dragenter", function(e) {
                return false
...
$('.lens')
        ...
        function() {
            ...
            if (document.running) {
                ...
                var countQ = function(query) {
                    var query = executeQ(query);
                    // execute query
                    qb = document.kb;
                    $.each(query.split('. '), function(index, value) {
                        if (value.match(/\(.*\)/))
                            qb = qb.optional(value.replace(/[\(\)]/g,
                                    ''))
                        else
                            qb = qb.where(value)
                    })
                ...
                var show = {
                    "literal-uri-anchor": function(v) {
                        // if not an URL just return a literal
                        if (!new RegExp(
                ...
                var query = (function() {
                    return executeQ($(self).attr('data-query'));
                })()

                var update = function() {
                    // generate kb query by disjunction and conjunction
                    // of
                    // triples
                    var rdfA = document.kb
                    if (document.voc)
                        rdfA = rdfA.add(document.voc)

                    var Q = undefined;

                    var queries = query.split(' OR ');

                    for (index = 0; index < queries.length; index++)
                        if (countQ(queries[index]) > 0) {
                            Q = queries[index];
                            break;
                        }
                    Q = (Q) ? Q : queries[queries.length - 1];

                    $.each(Q.split('. '), function(index, value) {
                        if (value.match(/\(.*\)/))
                            rdfA = rdfA.optional(value.replace(
                                    /[\(\)]/g, ''))
                        else
...
$.editableFactory.xsdString = {
    toEditable: function($this, options) {
        $('<input/>').appendTo($this).val(
                $this.data('editable.current').decodeLiteral())
    },
                        ...
                        onSubmit: function(content) {
                            if (content.previous != content.current) {
                                var reg = new RegExp(
                                        ...
                                        "\\$&"),
                                        "g")
                                var ttl = document.kb.databank
                                        .dump({
                                    format: "text/turtle"
                                })
                                var ttl2 = ttl.replace(reg,
                                        content.current)
                                document.kb = $.rdf()
                                        .load(ttl2)
                                document.kb
                                        ...
                                        .prefix('legal',
                                        'http://www.linked-usdl.org/ns/usdl-legal#')
                                        .prefix('price',
                                        'http://www.linked-usdl.org/ns/usdl-pricing#')
                                        .prefix('sla',
                                        'http://www.linked-usdl.org/ns/usdl-sla#')
                                        .prefix('sec',
                                        'http://www.linked-usdl.org/ns/usdl-sec#')
                                        .prefix('sec-tax',
                                        'http://www.linked-usdl.org/ns/usdl-sec-taxonomy#')
                                        .prefix('blueprint',
...
$('.selectEdit').livequery(
        function() {
            $(this).editable(
                    {
                        type: 'select',
                        editClass: 'valueColEdit',
                        options: $(this).attr("data-extend-snippet-id") ? $(
                                this).attr("options").concat("," + def).split(
                                ",") : $(this).attr("options").split(","),
                        onSubmit: function(content) {
                            var snippet = $(this)
                                    .parents(".snippetFrame:first").attr(
                                    "data-snippet-id")
                            ...
                            if (prefix) {
                                ...
                                oldTriple = subject + ' ' + property + ' "'
                                        + content.previous + '"' + type + lang
...
$.editableFactory.html = {
    toEditable: function($this, options) {
        ...
        var ta = $('<div/>').appendTo(ec).addClass('editable').html(
                $this.data('editable.current').decodeLiteral()).attr(
                'contentEditable', 'true').css('border', '1px inset #1D7DB3')
                .focus()

        var tb = $('<div class="easyEditorToolBar">')
                .append(
                $('<button title="Undo" class="button">⤾</button>')
                .click(function(e) {
            ...
            ta.focus()
                ...
                .append(
                $(
                '<button title="Bold" class="button" style="font-weight: bold;">B</button>')
                .click(function(e) {
            ...
            ta.focus()
                ...
                .append(
                $(
                '<button title="Italic" class="button" style="font-style:italic;">I</button>')
                .click(function(e) {
            document.execCommand('italic', null, null)
        }))
                .append(
                $(
                '<button title="Italic" class="button"><span style="text-decoration:underline">U</span></button>')
                .click(
                function(e) {
                ...
                .append(
                $(
                '<button title="Increase font size" class="button">A+</button>')
                .click(
                function(e) {
                ...
                .append(
                $(
                '<button title="Decrease font size" class="button">A-</button>')
                .click(
                function(e) {
                    document.execCommand(
                ...
                .append(
                $(
                '<button title="Remove format" class="button" data-alt="◯╳">∅</button>')
                .click(
                function(e) {
                    document.execCommand(
                ...
                /*
                 * .append($('<button title="Format as paragraph"
                 * class="button">P</button>') .click(function(e) {
                 * document.execCommand('insertParagraph', null, null) }))
                 */
                .append(
                $(
                '<button title="Unordered list" class="button">L</button>')
                .click(
                function(e) {
                    document.execCommand(
                            'insertUnorderedList',
                ...
                .append(
                $(
                '<button title="Ordered list" class="button">1.</button></div>')
                .click(
                function(e) {
                    document.execCommand(
            ...
            $(this).editable(
                    {
                        ...
                        onSubmit: function(content) {
                            ...
                            var oldTriple = subject
                                    ...
                                    + property
                                    + ' "'
                                    + $(this).data("content.orig")
                                    .encodeLiteral() + '"' + type
                                    + lang
                            var newTriple = subject + ' ' + property + ' "'
                                    + content.current.encodeLiteral() + '"'
                                    + type + lang
                            document.kb = document.kb.remove(oldTriple).add(
                                    newTriple)
                            document.kb.isDirty = true
            ...
            $(this).datetimepicker(
                    {
                        ...
                        onSelect: function(picked, inst) {
                            var snippet = $(this)
                                    ...
                                    "data-snippet-id")
                            var value = picked
                            var subject = encodeSubject($(input).attr(
                                    "data-subject"))
